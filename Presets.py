...
for i in range(9):
    ...
    img = pygame.image.load(path.join(img_dir, 'sonicExplosion0{}.png'.format(i)))
    img.set_colorkey((0, 0, 0))
    img_sm = pygame.transform.scale(img, (50, 50))
    exp_img['Mob_Boom'].append(img_sm)

for g in range(3):
    # filename = 'regularExplosion0{}.png'.format(i)
    img = pygame.image.load(path.join(img_dir, 'G{}.png'.format(g)))
    img.set_colorkey((0, 0, 0))
    img_sm = pygame.transform.scale(img, (50, 50))
    exp_img['G'].append(img_sm)
